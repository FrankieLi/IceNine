...
namespace CGAL {
  ...
  namespace XDM_test
  {
    ...
    public:
      ...
      template < class TDS6 >
      struct Rebind_TDS {
        typedef typename Vb::template Rebind_TDS<TDS6>::Other  Vb6;
        typedef XDM_vertex_base<GT, MD, Vb6>                   Other;
      };
      
      XDM_vertex_base():Vb(), fProtectingRadius(0) {}
      virtual ~XDM_vertex_base() {}

      void SetProtectingRadius( float f_ )
      {
        fProtectingRadius = f_;
      }

      float ProtectingRadius() const
      {
        return fProtectingRadius;
      }
      float fProtectingRadius;
    };
  }
  ...
  struct XDM_mesh_triangulation_3
  {
  private:
    typedef typename details::Mesh_geom_traits_generator<K>::type Geom_traits;
    typedef XDM_test::XDM_vertex_base<Geom_traits, MD >           Vertex_base;
  ...
  public:
    ...
    typedef type Type;
    typedef Cell_base Cell_type;   // for ID-ing purpose
  };  // end struct XDM_mesh_triangulation_3
